...
def update(id):
    ...
    if request.method == "POST":
        # Convert the input string to float before assigning to rating
        item_to_update.rating = float(request.form["rating"])
        item_to_update.review = request.form["review"]
        db.session.commit()
        return redirect(url_for("home"))

    return render_template("edit.html", form=form, id=id, name=item_to_update.title)
